...
client.on('message', async msg => {
	...
	if (msg.content === '!kms') {
		...
		if (imageSet.size == 0) images.forEach(item => imageSet.add(item))
		let random = Math.floor(Math.random() *imageSet.size);
		let link = Array.from(imageSet)[random];
		imageSet.delete(link);
